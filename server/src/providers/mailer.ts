...
let currentConfig: MailerConfig = {
  provider:
    (process.env.ZEPTOMAIL_API_KEY || process.env.ZEPT_API_KEY) &&
    !(process.env.ZEPTOMAIL_API_KEY || process.env.ZEPT_API_KEY || "").includes(
      "placeholder",
    )
      ? "zeptomail"
      : "ethereal",
...
export function updateMailerConfig(
  newConfig: Partial<MailerConfig>,
): MailerConfig {
...
export async function sendEmail(
  ...
  if (
    ...
    cfg.zeptoApiKey &&
    !cfg.zeptoApiKey.includes("placeholder")
  ) {
    ...
    try {
      ...
      console.log(
        "[ZEPTOMAIL SDK SUCCESS RESPONSE]:",
      ...
      console.error(
        "[ZEPTOMAIL SDK FAILED - RETRYING VIA DIRECT HTTPS REST API]:",
        sdkErr?.error || sdkErr?.message || sdkErr,
      );
      ...
      try {
        const fetchRes = await fetch(targetUrl, {
          method: "POST",
          headers: {
            Authorization: cfg.zeptoApiKey.startsWith("Zoho-enczapikey")
              ? cfg.zeptoApiKey
              : `Zoho-enczapikey ${cfg.zeptoApiKey}`,
        ...
        console.log(
          `[ZEPTOMAIL DIRECT REST API STATUS]: ${fetchRes.status} ${fetchRes.statusText}`,
        );
        console.log(
          "[ZEPTOMAIL DIRECT REST API BODY]:",
        ...
        console.error(
          "[ZEPTOMAIL DIRECT REST API ALSO FAILED]:",
        ...
        return {
          success: false,
          provider: "ZeptoMail (.IN API)",
          error: directErr?.message || sdkErr?.message || "ZeptoMail Error",
        };
      }
    }
  }
  ...
  try {
    ...
    console.log(
      `[SupermailBox Mailer] Delivered via Ethereal. Preview URL: ${previewUrl}`,
    );
