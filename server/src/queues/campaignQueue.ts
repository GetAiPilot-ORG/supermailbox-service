...
export const transactionalQueue = new Queue<CampaignJobPayload>(
...
export async function enqueueCampaignJobs(
  ...
  const bulkJobs = jobs.map((job) => ({
    name: job.productCode
      ? `[${job.productCode.toUpperCase()}] campaign-email`
  ...
  try {
    ...
    console.log(
      `📤 [BullMQ Enqueue] Added ${jobs.length} campaign job(s) to 'campaign-email-queue' in Redis.`,
    );
    return jobs.length;
  } catch (err) {
    console.error(
      "[Campaign Queue] Failed to enqueue jobs into Redis/BullMQ:",
      err,
    );
    throw err;
  }
...
export async function enqueueTransactionalJob(
  ...
  try {
    ...
    console.log(
      `⚡ [BullMQ Enqueue] Added high-priority job [${bullJob.id}] to 'transactional-email-queue' in Redis.`,
    );
    return bullJob.id || `tx_bull_${Date.now()}`;
  } catch (err) {
    console.error(
      "[Transactional Queue] Failed to enqueue transactional job into BullMQ:",
      err,
    );
    throw err;
  }
