...
function previewHtml(template: Seed) {
  const side = template.layout === 'receipt' ? '<table style="width:100%;border-collapse:collapse;color:#344054;font-size:14px"><tr><td>Reference</td><td align="right">{{order_id}}</td></tr><tr><td>Total</td><td align="right">{{amount}}</td></tr></table>' : '';
  const columns = template.layout === 'digest' ? '<div style="display:flex;flex-wrap:wrap;gap:14px;margin-top:18px"><div style="flex:1;min-width:140px"><b>Top story</b><p style="margin:5px 0">One primary update.</p></div><div style="flex:1;min-width:140px"><b>Quick note</b><p style="margin:5px 0">One supporting link.</p></div></div>' : '';
  return `<!DOCTYPE html><html><head><meta name="viewport" content="width=device-width, initial-scale=1.0"></head><body style="margin:0;padding:0"><div style="font-family:Arial,sans-serif;width:100%;max-width:600px;box-sizing:border-box;margin:0 auto;background:#eef2ef;padding:16px"><div style="background:#fff;border:1px solid #dfe4ea;border-radius:8px;overflow:hidden"><div style="height:${template.layout === 'commerce' || template.layout === 'hero' ? '86px' : '10px'};background:${template.accent}"></div><div style="padding:20px"><div style="color:${template.accent};font-size:12px;font-weight:700;text-transform:uppercase">${template.category}</div><h1 style="color:#25302a;font-size:24px;line-height:30px;margin:12px 0">${template.name}</h1><p style="color:#667085;font-size:15px;line-height:22px;margin:0 0 16px 0">${template.preheader}</p>${side}${columns}<a href="#" style="display:inline-block;background:${template.accent};color:#fff;text-decoration:none;border-radius:6px;padding:10px 16px;font-weight:700;margin-top:16px">Primary action</a></div></div></div></body></html>`;
}
